...
letterDictionaryEn = {'AEIOULNSTR':1,
                    'DG':2,
                    'BCMP':3,
                    'FHVWY':4,
                    'K':5,
...
letterDictionaryRu = {'АВЕИНОРСТ':1,
                     'ДКЛМПУ':2,
                     'БГЁЬЯ':3, 
                     'ЙЫ':4,
                     'ЖЗХЦЧ':5,
...
wordFormating = word.upper()

sum = 0
for i in wordFormating:
    for (k, v) in letterDictionaryEn.items() | letterDictionaryRu.items():
        if i in k:
            sum += v
print(f'{word} -> {sum}')